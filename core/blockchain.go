...
func (bc *BlockChain) GetFutureTxByContract(blocknumber uint64, idx uint) *types.Transaction{

	hashv:= rawdb.ReadEbnIndex(bc.db, idx, blocknumber)
	result := rawdb.ReadEbnTx(bc.db, idx, blocknumber, hashv)

	return result

}
func (bc *BlockChain) GetFutureIDX(blocknumber uint64) *uint{
	return rawdb.GetEbnIndex(bc.db,blocknumber)
